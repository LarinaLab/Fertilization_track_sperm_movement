...
for j=0:5  
    ...
    mkdir(strcat(TarfileDirectory,'continous8outof10_var5sep',num2str(j)));

        Vol(1:depth,1:width,1:TotalframeNumber,1:200+var_num+flaglen-2)=0;
        
        for fileNumber= 1:TotalframeNumber
            for series_num=1:200+var_num+flaglen-2
        ...
        Variance(1:depth,1:width,1:TotalframeNumber,1:200+flaglen-1)=0;
        mask(1:250,1:150,1:150,1:200+flaglen-1)=0;
        
        for varseries=1:200+flaglen-1
...
%         continousVar=normVar.*vol_continous;
  %% selecting for spot appear continously at next 8 out of 10 volumes
        vol_continous(1:250,1:150,1:150,1:209)=0;
        for temp1=1:250
                for temp2=1:150
                    for temp3=1:150
                        for temp4=1:200
...
%% output
        for m=1:200
            for n=1:150
                imwrite(uint8(255*mat2gray(continousVar(:,:,n,m))),strcat(TarfileDirectory,'continous8outof10_var5sep',num2str(j),'\',fileName1,'T',num2str(m),'_Z',num2str(n),'.tif'));
            end 
        end
    end 
